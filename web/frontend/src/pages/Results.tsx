...
export default function Results() {
    ...
    const completedJobs = jobs.filter(j => j.status === 'completed' && j.progress.scraped_places > 0)

    // Fetch results for selected job
    const { data: resultsData, isLoading } = useQuery({
        queryKey: ["results", selectedJobId, page, perPage],
        queryFn: () => jobsApi.getResults(selectedJobId, page, perPage),
    ...
    const filteredResults = useMemo(() => {
        ...
        if (!search.trim()) return resultsData.data

        const searchLower = search.toLowerCase()
        return resultsData.data.filter((entry) => {
            return (
    ...
    // Aggregate stats
    const totalResults = completedJobs.reduce((acc, j) => acc + j.progress.scraped_places, 0)

    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <h2 className="text-3xl font-bold tracking-tight">Results Browser</h2>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-3">
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{selectedJobId ? total.toLocaleString() : "-"}</div>
                    </CardContent>
                </Card>
            </div>
                            ...
                            >
                                ...
                                {completedJobs.map((job) => (
                                    <option key={job.id} value={job.id}>
                                        {job.name} ({job.progress.scraped_places} results)
                                    </option>
                                ))}
                            </select>
                        </div>
            ...
            {selectedJobId ? (
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between">
                        ...
                        <Button
                            variant="outline"
                            onClick={() => {
                                const url = jobsApi.downloadResults(selectedJobId, 'csv')
                ...
                <Card>
                    <CardContent className="py-12 text-center text-muted-foreground">
                        Select a job from the dropdown above to browse results.
                    </CardContent>
                </Card>
            )}
        </div>
